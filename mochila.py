...
def main ():
    pesos = []
    valores = []
    cantidad = int(input("Diga la cantidad de objetos: "))

    for i in range(cantidad):
        peso = int(input(f"Diga la peso del objeto (kg) {i + 1}: "))
        valor = int(input(f"Diga el valor del objeto ($) {i + 1}: "))

        pesos.append(peso)
        valores.append(valor)
        
    capacidad = int(input("Capacidad de la mochila (kg): "))
